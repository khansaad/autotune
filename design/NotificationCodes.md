...
|          |               |            |                  | Accelerator | 128000-128999   | API USER  |
...
|          |               |            |                  | Accelerator | 328000-328999   | DATA USER |
...
| 112104 |  INFO  |   RUNTIMES_RECOMMENDATION_AVAILABLE   |            Specifies that the Runtimes Recommendations are available            |                                       Runtimes Recommendations Available                                        | API USER  |
...
| 128001 |  INFO  | ACCELERATOR_RECOMMENDATIONS_AVAILABLE |            Specifies that Accelerator Recommendations are available             |                                    Accelerator Recommendations are available                                    | API USER  |
...
| 221005 | ERROR  |     NOT_ENOUGH_DATA_FOR_POD_COUNT     |    Specifies that there is not enough data available to determine pod count     |                                Not enough data available to determine Pod Count                                 | API USER  |
...
| 321001 | NOTICE |      POD_COUNT_DERIVED_FROM_CPU       |         Specifies that pod count is derived from CPU usage metric data          |                                 Pod count is derived from CPU usage metric data                                 | DATA USER |
| 321002 | NOTICE |     POD_COUNT_DERIVED_FROM_MEMORY     |        Specifies that pod count is derived from Memory usage metric data        |                               Pod count is derived from Memory usage metric data                                | DATA USER |
...
| 323006 | NOTICE |    CPU_REQUESTS_UNDER_PROVISIONED     |        Specifies that the workload is under-provisioned for CPU requests        |               Workload is under-provisioned for CPU. Kruize recommends increasing CPU allocation.               | DATA USER |
...
| 323008 | NOTICE |     CPU_LIMITS_UNDER_PROVISIONED      |         Specifies that the workload is under-provisioned for CPU limits         |        Workload is under-provisioned for CPU LIMITS. Kruize recommends increasing CPU limit allocation.         | DATA USER |
| 323009 | NOTICE |      CPU_LIMITS_OVER_PROVISIONED      |         Specifies that the workload is over-provisioned for CPU limits          | Workload is over-provisioned for CPU LIMITS. Kruize recommends reducing CPU limit allocation to optimise costs. | DATA USER |
...
| 324007 |  NOTICE  |   MEMORY_LIMITS_UNDER_PROVISIONED     |        Specifies that the workload is under-provisioned for Memory limits       |    Workload is under-provisioned for Memory LIMITS. Kruize recommends increasing Memory limit allocation.    | DATA USER |
| 324008 |  NOTICE  |    MEMORY_LIMITS_OVER_PROVISIONED     |        Specifies that the workload is over-provisioned for Memory limits        | Workload is over-provisioned for Memory LIMITS. Kruize recommends reducing Memory limit allocation to optimise costs. | DATA USER |
...
| 128001              |   INFO   | ACCELERATOR_RECOMMENDATIONS_AVAILABLE |                0                 |          0          |       0        |        1         |
...
| 112104              |   INFO   |   RUNTIMES_RECOMMENDATION_AVAILABLE   |                0                 |          0          |       1        |        0         |
