...
| `datasources[].cluster`                          | array  | No       | Array of cluster names associated with this datasource |
...
- **Optional Field**: If the `cluster` field is omitted, the datasource works without cluster association
- **Multiple Clusters**: Supports an array of cluster names
- **Backward Compatible**: Existing configurations without the cluster field continue to work
- **Empty Array**: An empty cluster array `[]` is treated the same as omitting the field
